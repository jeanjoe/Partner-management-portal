...
export class DocumentCategoryMappingService {
  ...
  getAllDocumentCotegory(): Observable<any> {
    return this.http.get(this.BASE_URL + URL.documentCategories).pipe(
      map((response) => {
        return response;
      })
    );
  ...
  getMappedDoc(code: string, lang: string): Observable<any> {
    return this.http
      .get(this.BASE_URL + URL.mappedDocUrl + code + "/" + lang)
      .pipe(
        map((response) => {
          return response;
        })
  ...
  getUnMappeddoc(): Observable<any> {
    return this.http
      .get(this.BASE_URL + URL.unMappedDocUrl + this.primaryLang)
      .pipe(
        map((response) => {
          return response;
        })
