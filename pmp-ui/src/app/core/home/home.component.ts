...
export class HomeComponent implements OnInit {
        ...
        .subscribe(({ response }) => {
          if (response.emailExists) {
            ...
            if (this.headerService.getPartnerType()) {
              if (
                response.policyRequiredPartnerTypes.indexOf(
                  this.headerService.getPartnerType()
                ) > -1
              ) {
                self.openFilterDialog({ case: "filter" });
              } else {
                let request = new RequestModel("", null, {
                  partnerId: this.headerService.getUsername(),
                  organizationName: this.headerService.getOrganizationName(),
                  address: this.headerService.getAddress(),
                  contactNumber: this.headerService.getContactNumber(),
                  emailId: this.headerService.getEmailId(),
                  partnerType: this.headerService.getPartnerType(),
                  langCode: this.headerService.getNotificationLanguage(),
                });
                this.dataService
  ...
  redirecttoupload() {
    this.router.navigateByUrl(
      "/pmp/resources/selfcert/upload/" + this.partnerIdDetail.partnerID
    );
  }
